...
@Getter
@Setter
public class MMXU_DZ extends LN {
    /**
     ...
     * Частотомер
     */
    public double[] currentPhsA = new double[]{0.0, 0.0};
    public double[] timerOhsA = new double[]{0.0, 0.0};
    public double valueY = 0.0;
    public double valueX = 0.0;
    public double dt = 0.00025; // = 0.02/80
    public int countFrecMeasu = 0;
    public double[] frecPointValue = new double[]{0.0, 0.0};
    public MV frequency = new MV();

    public MMXU_DZ() {
        this.frequency.getInstMag().getFloatVal().setValue(50.0);
    }
    ...
    public void process() {
        ...
        Vector valVoltAB = resultVector(this.PNV.getPhsA(), this.PNV.getPhsB());
        Vector valVoltBC = resultVector(this.PNV.getPhsB(), this.PNV.getPhsC());
        Vector valVoltCA = resultVector(this.PNV.getPhsC(), this.PNV.getPhsA());

        this.PPV.getPhsAB().setInstCVal(valVoltAB);
        this.PPV.getPhsBC().setInstCVal(valVoltBC);
        this.PPV.getPhsCA().setInstCVal(valVoltCA);

        /**Расчет сопротивлений:
         * в фазные переменные записаны междуфазные сопротивления, так как реализуется междуфазное подключение.
         * Междуфазное напряжение делится на разность токов, соответствующих фаз
         * */
        Vector valCurrAB = resultVector(this.A.getPhsA(), this.A.getPhsB());
        Vector valCurrBC = resultVector(this.A.getPhsB(), this.A.getPhsC());
        Vector valCurrCA = resultVector(this.A.getPhsC(), this.A.getPhsA());

        Vector valA = divCalc(valVoltAB, valCurrAB);
        Vector valB = divCalc(valVoltBC, valCurrBC);
        Vector valC = divCalc(valVoltCA, valCurrCA);

        this.Z.getPhsA().getCVal().getMag().getFloatVal().setValue(valA.getMag().getFloatVal().getValue());
        this.Z.getPhsA().getCVal().getAng().getFloatVal().setValue(valA.getAng().getFloatVal().getValue());
        this.Z.getPhsB().getCVal().getMag().getFloatVal().setValue(valB.getMag().getFloatVal().getValue());
        this.Z.getPhsB().getCVal().getAng().getFloatVal().setValue(valB.getAng().getFloatVal().getValue());
        this.Z.getPhsC().getCVal().getMag().getFloatVal().setValue(valC.getMag().getFloatVal().getValue());
        this.Z.getPhsC().getCVal().getAng().getFloatVal().setValue(valC.getAng().getFloatVal().getValue());
        /**Замер частоты*/
        freqMeasur(frequency);
        System.out.println("Частота в сети: " + frequency.getInstMag().getFloatVal().getValue());
    }
    ...
    /**
     * Возвращает вектор междуфазного сопротивления
     */
    public Vector divCalc(Vector a, Vector b) {
        double reA = a.getMag().getFloatVal().getValue() * Math.cos(a.getAng().getFloatVal().getValue());
        double reB = b.getMag().getFloatVal().getValue() * Math.cos(b.getAng().getFloatVal().getValue());
        double imA = a.getMag().getFloatVal().getValue() * Math.sin(a.getAng().getFloatVal().getValue());
        double imB = b.getMag().getFloatVal().getValue() * Math.sin(b.getAng().getFloatVal().getValue());

        //Математические формулы деления комплексных чисел
        double realRes = (reA * reB + imA * imB) / (reB * reB + imB * imB);
        double imagRes = (reB * imA - reA * imB) / (reB * reB + imB * imB);

        Vector result = new Vector();

        result.getMag().getFloatVal().setValue(Math.sqrt(realRes * realRes + imagRes * imagRes));

        if (realRes > 0) {
            result.getAng().getFloatVal().setValue(Math.atan(imagRes / realRes));
        } else if (realRes < 0 && imagRes >= 0) {
            result.getAng().getFloatVal().setValue(Math.PI + Math.atan(imagRes / realRes));
        } else if (realRes < 0 && imagRes < 0) {
            result.getAng().getFloatVal().setValue(-Math.PI + Math.atan(imagRes / realRes));
        } else if (realRes == 0 && imagRes > 0) {
            result.getAng().getFloatVal().setValue(Math.PI / 2);
        } else if (realRes == 0 && imagRes < 0) {
            result.getAng().getFloatVal().setValue(-Math.PI / 2);
        }
        return result;
    ...
    /**
     * Функция замера частоты по пересечению графика нуля с использованием интерполяции
     */
    public void freqMeasur(MV frequency) {
        currentPhsA[1] = currentPhsA[0];
        currentPhsA[0] = IaInst.getInstMag().getFloatVal().getValue();

        timerOhsA[1] = timerOhsA[0];
        timerOhsA[0] = countFrecMeasu * dt;

        if (currentPhsA[0] * currentPhsA[1] <= 0) {
            valueX = ((valueY - currentPhsA[1]) / (currentPhsA[0] - currentPhsA[1])) * (timerOhsA[0] - timerOhsA[1]) + timerOhsA[1];
            frecPointValue[1] = frecPointValue[0];
            frecPointValue[0] = valueX;
            if (!(frecPointValue[1] == 0.0)) {
                double freq = 1 / (2 * (frecPointValue[0] - frecPointValue[1]));
                frequency.getInstMag().getFloatVal().setValue(freq);
            }
        }
        countFrecMeasu++;
    }
